    ...
    services.factory('ActivityService', function ($http, ipCookie) {
        return{
            addVoteToDbAndToCookie: function (problemID,userID,userName,userSurname) {
                ...
                return $http.post('http://www.ecomap.org/api/vote', {idProblem: problemID, userId: userID, userName: userName,userSurname:userSurname})
            ...
            addCommentToDB:function(userID,userName,userSurname,problemID,comment,updateScope) {
                ...
                return $http.post('http://www.ecomap.org/api/comment/' + problemID, JSON.stringify(data))
                .success(function (data, status, headers, config) {
            ...
            deleteCommentFromDB:function(id){
                return $http.delete('http://www.ecomap.org/api/activity/' + id)
                    .success(function (data, status, headers, config) {
                          })
                    .error(function (data, status, headers, config) {
                       throw error;
